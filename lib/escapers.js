...
function escapeId(val, forbidQualified) {
  if (typeof val === 'object') {
    if (isSqlId(val)) {
      ...
      escapeSeries(val, (element) => escapeId(element, forbidQualified), false);
    }
  }
  if (forbidQualified) {
...
function escape(val, stringifyObjects, timeZone) {
  ...
  if (isSeries(val)) {
    return escapeSeries(val, (element) => escape(element, stringifyObjects, timeZone), true);
  }
...
function escapeString(val) {
  ...
  for (let match; (match = CHARS_GLOBAL_REGEXP.exec(val));) {
