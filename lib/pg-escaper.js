...
// Note: NULs are not allowed in text data value.
// https://www.postgresql.org/docs/current/sql-syntax-lexical.html#SQL-SYNTAX-STRINGS-ESCAPE says
...
function pgEscapeId(val, forbidQualified, unicode) {
  ...
  let escaped = unicode ?
    ...
    pgEscapeStringBody(`${ val }`, PG_ID_ESCAPE_MAP);
  if (!forbidQualified) {
...
function pgEscapeDelimitedString(strValue, delimiter) {
  switch (delimiter) {
    ...
    case 'b\'':
    case 'x\'':
      return pgEscapeStringBody(strValue, PG_CHARS_ESCAPE_MAP);
    case 'e\'':
