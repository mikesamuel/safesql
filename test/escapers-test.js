...
describe('escapeId', () => {
  it('value is quoted', () => {
    expect(escapeId('id')).to.equal('`id`');
  });

  it('value can be a number', () => {
    expect(escapeId(42)).to.equal('`42`');
  });

  it('value can be an object', () => {
    expect(escapeId({})).to.equal('`[object Object]`');
  });
  ...
  it('value containing escapes is quoted', () => {
    expect(escapeId('i`d')).to.equal('`i``d`');
  });

  it('value containing separator is quoted', () => {
    expect(escapeId('id1.id2')).to.equal('`id1`.`id2`');
  });

  it('value containing separator and escapes is quoted', () => {
    expect(escapeId('id`1.i`d2')).to.equal('`id``1`.`i``d2`');
  });

  it('value containing separator is fully escaped when forbidQualified', () => {
    expect(escapeId('id1.id2', true)).to.equal('`id1.id2`');
  });

  it('arrays are turned into lists', () => {
    expect(escapeId([ 'a', 'b', 't.c' ])).to.equal('`a`, `b`, `t`.`c`');
  });
...
describe('escape', () => {
  ...
  it('nested objects are cast to strings', () => {
    expect(escape({ a: { nested: true } })).to.equal('`a` = \'[object Object]\'');
  });
